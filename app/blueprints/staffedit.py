...
from config import CSV_FILE,PROJECT_FILE,pepo,renliprompt,CV_FILE, CV_EN_FILE
from datetime import datetime
from collections import defaultdict
from app.utils.utils import  chat_mode_staff
import json
...
def reset_renli():
    ...
    initial_history = [{"role": "system", "content": renliprompt}]
...
def reset_pepo():
    ...
    initial_history = [{"role": "system", "content": pepo}]
    new_history_entry = History(user_id=user_id, session_id=str(uuid.uuid4()), messages=json.dumps(initial_history))
    db.session.add(new_history_entry)
    db.session.commit()
...
```'''
    postai = Selected_staff + prostr + resback
